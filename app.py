...
def _id3_text_frame(fid, text):
    if not text:
        return b""
    data = b"\x03" + str(text).encode("utf-8")   # encoding byte: UTF-8
    return fid.encode() + _id3_struct.pack(">I", len(data)) + b"\x00\x00" + data

def _id3_comm_frame(text, lang="eng"):
    data = b"\x03" + lang.encode() + b"\x00" + str(text).encode("utf-8")
    return b"COMM" + _id3_struct.pack(">I", len(data)) + b"\x00\x00" + data

def _id3_apic_frame(image_bytes, mime="image/jpeg"):
    """Attached-picture frame, type 0x03 = cover (front), empty description."""
    data = b"\x00" + mime.encode() + b"\x00" + b"\x03" + b"\x00" + image_bytes
    return b"APIC" + _id3_struct.pack(">I", len(data)) + b"\x00\x00" + data

def _id3_chap_frame(elem_id, start_ms, end_ms, title):
    """CHAP chapter frame with an embedded TIT2 title subframe (ID3 chapter addendum)."""
    sub = _id3_text_frame("TIT2", title)
    data = (elem_id.encode("ascii", "replace") + b"\x00"
            + _id3_struct.pack(">I", max(0, int(start_ms)))
            + _id3_struct.pack(">I", max(0, int(end_ms)))
            + _id3_struct.pack(">I", 0xFFFFFFFF)   # start byte offset: unused
            + _id3_struct.pack(">I", 0xFFFFFFFF)   # end byte offset: unused
            + sub)
    return b"CHAP" + _id3_struct.pack(">I", len(data)) + b"\x00\x00" + data

def _id3_ctoc_frame(elem_id, child_ids, title):
    """CTOC table-of-contents frame — top-level + ordered (flags 0x03)."""
    data = (elem_id.encode("ascii", "replace") + b"\x00"
            + bytes([0x03]) + bytes([len(child_ids) & 0xFF]))
    for cid in child_ids:
        data += cid.encode("ascii", "replace") + b"\x00"
    data += _id3_text_frame("TIT2", title)
    return b"CTOC" + _id3_struct.pack(">I", len(data)) + b"\x00\x00" + data

def _build_id3v2_tag(title="", artist="", album="", year="", comment="",
                     track="", cover_bytes=None, cover_mime="image/jpeg",
                     chapters=None):
    """Return a complete ID3v2.3 tag block (bytes).
    chapters: optional list of (elem_id, start_ms, end_ms, chapter_title)."""
    frames = (_id3_text_frame("TIT2", title)
              + _id3_text_frame("TPE1", artist)
              + _id3_text_frame("TALB", album)
              + _id3_text_frame("TYER", year))
    if track:
        frames += _id3_text_frame("TRCK", track)
    if comment:
        frames += _id3_comm_frame(comment)
    if cover_bytes:
        frames += _id3_apic_frame(cover_bytes, cover_mime)
    if chapters:
        frames += _id3_ctoc_frame("toc", [c[0] for c in chapters], album or title)
        for cid, s, e, t in chapters:
            frames += _id3_chap_frame(cid, s, e, t)
    return b"ID3\x03\x00\x00" + _id3_syncsafe(len(frames)) + frames
...
def _write_id3v2(filepath, title="", artist="", album="", year="", comment=""):
    ...
    tag = _build_id3v2_tag(title=title, artist=artist, album=album,
                           year=year, comment=comment)
    with open(filepath, "r+b") as f:
        mp3_data = f.read()
        f.seek(0)
        f.write(tag + mp3_data)
        f.truncate()
...
# ══════════════════════════════════════════════════════════════════════════════
# AUDIOBOOK ENGINE — split long text into chapters, build a chaptered MP3 and/or
# a folder of per-chapter MP3s. Pure-Python tags (no ffmpeg/mutagen). Each chapter
# is generated, loudness-normalised and checkpointed to disk (bounded memory +
# crash-resume) before the cheap assembly step.
# ══════════════════════════════════════════════════════════════════════════════
_AB_SR   = 24000
_AB_LUFS = -16.0

_CHAP_HEAD_RE = re.compile(
    r'^\s*(chapter|part|book|section|volume|prologue|epilogue|introduction'
    r'|preface|foreword|afterword|appendix|interlude)\b[\s.:\-—]*(.*)$',
    re.IGNORECASE)
_MD_HEAD_RE = re.compile(r'^\s{0,3}(#{1,3})\s+(.+?)\s*#*\s*$')

def _ab_clean_heading(line):
    m = _MD_HEAD_RE.match(line)
    if m:
        return m.group(2).strip()
    return line.strip().lstrip('#').strip()

def _ab_is_heading(line):
    s = line.strip()
    if not s:
        return False
    if _MD_HEAD_RE.match(line):
        return True
    if _CHAP_HEAD_RE.match(s) and len(s) <= 80:
        return True
    return False

def split_into_chapters(text, default_title="Audiobook"):
    """Heuristically split text into [(title, content_without_heading), ...].
    Detects 'Chapter N', 'Part Two', 'Prologue', etc. and Markdown #/##/###
    headings. Prose before the first heading becomes an 'Introduction'. If no
    headings are found, returns a single chapter holding the whole text."""
    if not text or not text.strip():
        return [(default_title, "")]
    lines = text.splitlines()
    chapters, pre, cur_title, cur_body = [], [], None, []
    for line in lines:
        if _ab_is_heading(line):
            if cur_title is not None:
                ...
                pre = []
            cur_title = _ab_clean_heading(line) or f"Chapter {len(chapters) + 1}"
            cur_body = []
        else:
            (cur_body if cur_title is not None else pre).append(line)
    if cur_title is not None:
        chapters.append([cur_title, cur_body])
    elif pre:
        chapters.append([default_title, pre])
    out = []
    for i, (title, body) in enumerate(chapters):
        content = "\n".join(body).strip()
        out.append((title or f"Chapter {i + 1}", content))
    non_empty = [(t, c) for (t, c) in out if c.strip()] or [(out[0][0], out[0][1])]
    return non_empty

def split_by_size(text, target_chars=4000, default_title="Chapter"):
    """Fallback split (no headings): group paragraphs into ~target_chars chapters."""
    paras = re.split(r'\n\s*\n', text.strip())
    ...
    for p in paras:
        ...
        if size >= target_chars:
            n += 1
            chapters.append((f"{default_title} {n}", "\n\n".join(buf)))
            buf, size = [], 0
    if buf:
        n += 1
        chapters.append((f"{default_title} {n}", "\n\n".join(buf)))
    return chapters or [(f"{default_title} 1", text.strip())]

def _ab_safe_filename(name, fallback="chapter"):
    name = re.sub(r'[<>:"/\\|?*\x00-\x1f]', "_", str(name)).strip().strip(".")
    name = re.sub(r"\s+", " ", name)
    return (name or fallback)[:80]

def _ab_encode_mp3_bytes(samples, sr, bitrate):
    """Encode float samples to raw MP3 bytes (mono), no tags."""
    import lameenc
    pcm = np.clip(samples, -1.0, 1.0)
    pcm = (pcm * 32767).astype(np.int16)
    if pcm.ndim > 1:
...
def _ab_prepare_cover(image_path, size=600):
    """Load a user image, centre-crop square, resize, return (jpeg_bytes, mime)."""
    try:
...
def _build_audiobook(chapters, *, voice, speed, lang, out_dir, book_title,
                     author="", year="", cover_bytes=None, cover_mime="image/jpeg",
                     make_folder=True, make_single=True, speak_titles=True,
                     gap_s=0.7, bitrate=128, work_dir=None,
                     progress_cb=None, should_cancel=None, cleanup=True):
    """Generate a multi-chapter audiobook. chapters: list of (title, content).
    Returns {'folder': path|None, 'single': path|None, 'chapters': [...],
             'cancelled': bool}. Only one chapter's audio is held in RAM."""
    def emit(event, **kw):
        if progress_cb:
            try: progress_cb(event, **kw)
            except Exception: pass
    def cancelled():
    ...
    if work_dir is None:
        work_dir = os.path.join(out_dir, ".vw_audiobook_tmp")
    os.makedirs(work_dir, exist_ok=True)

    n = len(chapters)
    weights = [max(1, len(t or "") + len(c or "")) for t, c in chapters]
    emit("begin", weights=weights)

    # ── 1. generate + checkpoint each chapter ──────────────────────────────
    durations = []
    sig = f"{voice}|{speed}|{bitrate}|{int(speak_titles)}|{lang}"
    for i, (title, content) in enumerate(chapters):
        if cancelled():
            return {"folder": None, "single": None, "chapters": [], "cancelled": True}
        ck_mp3  = os.path.join(work_dir, f"chap_{i:04d}.mp3")
        ck_meta = os.path.join(work_dir, f"chap_{i:04d}.json")
        if os.path.exists(ck_mp3) and os.path.exists(ck_meta):
            try:
                m = json.load(open(ck_meta, encoding="utf-8"))
                if m.get("sig") == sig and m.get("title") == title and m.get("done"):
                    durations.append(m["duration_ms"])
                    emit("chapter_done", i=i, reused=True)
                    continue
            except Exception:
                pass
        emit("chapter_start", i=i, title=title)
        spoken = content
        if speak_titles and title:
            spoken = f"{title}.\n\n{content}".strip()
        parts = []
        ...
        try:
            audio, _info = normalize_loudness(audio, _AB_SR, _AB_LUFS)
            audio = np.asarray(audio, dtype=np.float32)
        except Exception as e:
            _log_crash(e)
        dur_ms = int(round(len(audio) / _AB_SR * 1000))
        with open(ck_mp3, "wb") as f:
            f.write(_ab_encode_mp3_bytes(audio, _AB_SR, bitrate))
        json.dump({"sig": sig, "title": title, "duration_ms": dur_ms, "done": True},
                  open(ck_meta, "w", encoding="utf-8"))
        durations.append(dur_ms)
        del audio, parts
        emit("chapter_done", i=i)

    emit("assembling")
    safe_book = _ab_safe_filename(book_title, "Audiobook")
    result = {"folder": None, "single": None, "cancelled": False,
              "chapters": [(chapters[i][0], durations[i]) for i in range(n)]}

    # ── 2. folder of per-chapter tagged MP3s (max compatibility) ───────────
    if make_folder:
        folder = os.path.join(out_dir, safe_book)
        os.makedirs(folder, exist_ok=True)
        for i, (title, _c) in enumerate(chapters):
            with open(os.path.join(work_dir, f"chap_{i:04d}.mp3"), "rb") as f:
                audio_bytes = f.read()
            tag = _build_id3v2_tag(title=title, artist=author, album=book_title,
                                   year=year, track=f"{i+1}/{n}",
                                   comment=f"Generated by VoxWild v{VERSION}",
                                   cover_bytes=cover_bytes, cover_mime=cover_mime)
            fn = os.path.join(folder, f"{i+1:02d} - {_ab_safe_filename(title)}.mp3")
            with open(fn, "wb") as f:
                f.write(tag + audio_bytes)
        result["folder"] = folder

    # ── 3. single chaptered MP3 (ID3 CHAP/CTOC) ────────────────────────────
    if make_single:
        gap_ms  = int(round(gap_s * 1000))
        gap_mp3 = (_ab_encode_mp3_bytes(np.zeros(int(gap_s * _AB_SR), dtype=np.float32),
                                        _AB_SR, bitrate) if gap_s > 0 else b"")
        markers, cursor = [], 0
        for i in range(n):
            start = cursor
            cursor += durations[i]
            if i < n - 1:
                cursor += gap_ms
            markers.append((f"chp{i}", start, cursor, chapters[i][0]))
        tag = _build_id3v2_tag(title=book_title, artist=author, album=book_title,
                               year=year, comment=f"Generated by VoxWild v{VERSION}",
                               cover_bytes=cover_bytes, cover_mime=cover_mime,
                               chapters=markers)
        single = os.path.join(out_dir, f"{safe_book}.mp3")
        with open(single, "wb") as out:
            out.write(tag)
            for i in range(n):
                with open(os.path.join(work_dir, f"chap_{i:04d}.mp3"), "rb") as f:
                    out.write(f.read())
                if i < n - 1 and gap_mp3:
                    out.write(gap_mp3)
        result["single"] = single

    # ── 4. cleanup checkpoints on success ──────────────────────────────────
    if cleanup:
        try:
...
# AUDIOBOOK TAB
# ══════════════════════════════════════════════════════════════════════════════
ab_tab = tabs.tab("  Audiobook  ")
ab_tab.configure(fg_color=C_BG)
ab_tab.grid_columnconfigure(0, weight=3)      # book text
ab_tab.grid_columnconfigure(1, minsize=286)   # chapter preview
ab_tab.grid_columnconfigure(2, minsize=300)   # details / build
ab_tab.grid_rowconfigure(0, weight=1)

# ── shared state ──
ab_chapters = []                       # [{"title_var": StringVar, "content": str}]
ab_cover    = {"bytes": None, "mime": "image/jpeg", "path": ""}
ab_voice_var        = ctk.StringVar(value="🇺🇸 Female - Heart (Best)")
ab_title_var        = ctk.StringVar(value="")
ab_author_var       = ctk.StringVar(value="")
ab_year_var         = ctk.StringVar(value=datetime.now().strftime("%Y"))
ab_fmt_var          = ctk.StringVar(value="Both")
ab_speak_titles_var = ctk.BooleanVar(value=True)

# ═══ Column 0 — book text ═══
ab_text_panel = ctk.CTkFrame(ab_tab, fg_color=C_CARD, corner_radius=12)
ab_text_panel.grid(row=0, column=0, sticky="nsew", padx=(0, 8), pady=6)
_section_label(ab_text_panel, "BOOK TEXT",
    tooltip="Paste or import your whole book. Chapters are auto-detected from lines "
            "like 'Chapter 3', 'Part Two', 'Prologue', and Markdown headings (#, ##). "
            "Click Detect Chapters to preview and edit the split.")
ab_text = ctk.CTkTextbox(ab_text_panel,
                         font=ctk.CTkFont(family="Consolas", size=12),
...
ab_detect_btn = ctk.CTkButton(ab_txt_btns, text="Detect Chapters  ▸", width=150, height=28,
              ...
              font=ctk.CTkFont(family="Segoe UI", size=12, weight="bold"),
              corner_radius=8)
ab_detect_btn.pack(side="right")

# ═══ Column 1 — chapter preview ═══
ab_chap_panel = ctk.CTkFrame(ab_tab, fg_color=C_CARD, corner_radius=12)
ab_chap_panel.grid(row=0, column=1, sticky="nsew", padx=(0, 8), pady=6)
_section_label(ab_chap_panel, "CHAPTERS",
    tooltip="Detected chapters. Edit any title inline — it becomes the chapter name "
            "in the audiobook. To change where chapters break, edit your text (add "
            "'Chapter N' lines or # headings) and Detect again.")
ab_chap_summary = ctk.CTkLabel(ab_chap_panel,
    text="Paste or import text, then Detect Chapters.",
    font=ctk.CTkFont(family="Segoe UI", size=10), text_color=C_TXT2,
    wraplength=258, justify="left", anchor="w")
ab_chap_summary.pack(fill="x", padx=16, pady=(0, 6))
_sep(ab_chap_panel)
ab_chap_scroll = ctk.CTkScrollableFrame(ab_chap_panel, fg_color="transparent")
ab_chap_scroll.pack(fill="both", expand=True, padx=6, pady=6)

# ═══ Column 2 — book details + build ═══
ab_side = ctk.CTkFrame(ab_tab, fg_color=C_CARD, corner_radius=12)
ab_side.grid(row=0, column=2, sticky="nsew", padx=0, pady=6)
# footer pinned to the bottom (packed first)
ab_foot = ctk.CTkFrame(ab_side, fg_color="transparent")
ab_foot.pack(side="bottom", fill="x", padx=14, pady=(6, 12))
ab_build_btn = ctk.CTkButton(ab_foot, text="🎧  Build Audiobook", height=40,
              fg_color=C_ACCENT, hover_color=C_ACCENT_H, text_color="#00120b",
              font=ctk.CTkFont(family="Segoe UI", size=14, weight="bold"),
              corner_radius=10)
ab_build_btn.pack(fill="x")
ab_cancel_btn = ctk.CTkButton(ab_foot, text="Cancel", height=28, corner_radius=8,
              state="disabled", **BTN_DANGER)
ab_cancel_btn.pack(fill="x", pady=(6, 0))
ctk.CTkFrame(ab_side, fg_color=C_BORDER, height=1, corner_radius=0).pack(side="bottom", fill="x")
# scrollable body above the footer
ab_details = ctk.CTkScrollableFrame(ab_side, fg_color="transparent")
ab_details.pack(side="top", fill="both", expand=True)

def _ab_field(parent, label, var, placeholder=""):
    ctk.CTkLabel(parent, text=label,
                 font=ctk.CTkFont(family="Segoe UI", size=11),
                 text_color=C_TXT2, anchor="w").pack(fill="x", padx=14, pady=(8, 1))
    e = ctk.CTkEntry(parent, textvariable=var, fg_color=C_BG,
...
_ab_field(ab_details, "Title", ab_title_var, placeholder="e.g. The Lighthouse")
_ab_field(ab_details, "Author", ab_author_var, placeholder="e.g. Jane Doe")
_ab_field(ab_details, "Year", ab_year_var)

ctk.CTkLabel(ab_details, text="Cover art",
             font=ctk.CTkFont(family="Segoe UI", size=11),
             text_color=C_TXT2, anchor="w").pack(fill="x", padx=14, pady=(8, 1))
ab_cover_btn = ctk.CTkButton(ab_details, text="Choose image…", height=30,
             corner_radius=8, **BTN_DARK)
ab_cover_btn.pack(fill="x", padx=14)
ab_cover_label = ctk.CTkLabel(ab_details, text="optional",
             font=ctk.CTkFont(family="Segoe UI", size=10),
             text_color=C_TXT3, anchor="w")
ab_cover_label.pack(fill="x", padx=16, pady=(2, 0))

ctk.CTkFrame(ab_details, fg_color="transparent", height=6).pack()
_section_label(ab_details, "NARRATION")
ctk.CTkLabel(ab_details, text="Voice",
             font=ctk.CTkFont(family="Segoe UI", size=11),
             text_color=C_TXT2, anchor="w").pack(fill="x", padx=14, pady=(4, 1))
ctk.CTkOptionMenu(ab_details, variable=ab_voice_var, values=list(VOICES.keys()),
             dynamic_resizing=False,
             font=ctk.CTkFont(family="Segoe UI", size=12)).pack(fill="x", padx=14)
ab_speed_row = ctk.CTkFrame(ab_details, fg_color="transparent")
ab_speed_row.pack(fill="x", padx=14, pady=(8, 2))
ctk.CTkLabel(ab_speed_row, text="Narration speed",
             font=ctk.CTkFont(family="Segoe UI", size=11),
             text_color=C_TXT2, anchor="w").pack(side="left")
ab_speed_val = ctk.CTkLabel(ab_speed_row, text="1.00×", text_color=C_ACCENT,
             width=52, anchor="e",
             font=ctk.CTkFont(family="Segoe UI", size=11))
ab_speed_val.pack(side="right")
ab_speed = ctk.CTkSlider(ab_details, from_=0.5, to=2.0, number_of_steps=30, height=14,
             command=lambda v: ab_speed_val.configure(text=f"{float(v):.2f}×"))
ab_speed.set(1.0)
ab_speed.pack(fill="x", padx=14, pady=(0, 8))
ctk.CTkCheckBox(ab_details, text="Speak chapter titles", variable=ab_speak_titles_var,
             font=ctk.CTkFont(family="Segoe UI", size=11), text_color=C_TXT2,
             checkbox_width=18, checkbox_height=18).pack(anchor="w", padx=14, pady=(2, 4))

ctk.CTkFrame(ab_details, fg_color="transparent", height=6).pack()
_section_label(ab_details, "OUTPUT",
    tooltip="Folder = one MP3 per chapter (works in every audiobook app — recommended). "
            "Single MP3 = one file with embedded chapter markers (Voice, VLC, some apps). "
            "Both gives you each.")
ctk.CTkSegmentedButton(ab_details, values=["Folder", "Single MP3", "Both"],
             variable=ab_fmt_var,
             font=ctk.CTkFont(family="Segoe UI", size=11)).pack(fill="x", padx=14, pady=(2, 4))
ctk.CTkLabel(ab_details,
             text="Fast mode · one narrator voice. Long books can take a while — "
                  "progress and time remaining show in the bar below.",
             font=ctk.CTkFont(family="Segoe UI", size=10), text_color=C_TXT3,
             wraplength=252, justify="left", anchor="w").pack(fill="x", padx=14, pady=(4, 10))

# ── handlers ──
def ab_import_file():
    folder = get_default_folder()
    path = filedialog.askopenfilename(title="Import book text",
        initialdir=folder or None,
    ...
    if not ab_title_var.get().strip():
        ab_title_var.set(os.path.splitext(os.path.basename(path))[0])
    ab_detect_chapters()
...
def ab_pick_cover():
    ...
    if not path:
        return
    b, mime = _ab_prepare_cover(path)
    if not b:
        status_label.configure(text="⚠️ Could not read that image.")
        return
    ab_cover["bytes"], ab_cover["mime"], ab_cover["path"] = b, mime, path
    ab_cover_label.configure(text="🖼  " + os.path.basename(path), text_color=C_TXT2)

def _ab_clear_rows():
    for w in ab_chap_scroll.winfo_children():
        w.destroy()
    ab_chapters.clear()
...
def ab_detect_chapters(*_):
    ...
    for i, (title, content) in enumerate(detected):
        row = ctk.CTkFrame(ab_chap_scroll, fg_color=C_BG, corner_radius=8)
        row.pack(fill="x", padx=4, pady=3)
        top = ctk.CTkFrame(row, fg_color="transparent")
        top.pack(fill="x", padx=8, pady=(6, 0))
        ctk.CTkLabel(top, text=f"{i+1:02d}", width=22, text_color=C_ACCENT,
                     font=ctk.CTkFont(family="Segoe UI", size=11, weight="bold")).pack(side="left")
        tv = ctk.StringVar(value=title)
        ent = ctk.CTkEntry(top, textvariable=tv, height=26, fg_color=C_CARD,
                           border_color=C_BORDER, text_color=C_TXT,
                           font=ctk.CTkFont(family="Segoe UI", size=12))
        ...
        ctk.CTkLabel(row, text=f"{words:,} words", anchor="w", text_color=C_TXT3,
                     ...
                     anchor="w", padx=(36, 8), pady=(1, 6))
        ab_chapters.append({"title_var": tv, "content": content})
    total_words = sum(len(c["content"].split()) for c in ab_chapters)
    try:
        secs = sum(estimate_audio_duration(c["content"], ab_speed.get()) for c in ab_chapters)
        dur = format_time(secs)
    except Exception:
        dur = "—"
    ab_chap_summary.configure(
...
def _ab_reset_buttons():
    ab_build_btn.configure(state="normal", text="🎧  Build Audiobook")
    ab_cancel_btn.configure(state="disabled", text="Cancel")
    ab_detect_btn.configure(state="normal")

def _ab_done_ui(res, out_dir):
    n = len(res.get("chapters", []))
    status_label.configure(text=f"✅ Audiobook ready — {n} chapters saved.")
    msg = "Your audiobook is ready.\n\n"
    if res.get("folder"):
        msg += f"Chapters folder:\n{res['folder']}\n\n"
    if res.get("single"):
        msg += f"Single file:\n{res['single']}\n\n"
    msg += "Open the output folder now?"
    try:
        if messagebox.askyesno("Audiobook created", msg):
...
def ab_build():
    global is_generating
    if is_generating or _preview_busy[0]:
        status_label.configure(text="⚠️ Already generating — wait for it to finish.")
        return
    text = ab_text.get("1.0", "end").strip()
    if not text:
        status_label.configure(text="⚠️ Add some book text first.")
        return
    if not ab_chapters:
        ab_detect_chapters()
    if not ab_chapters:
        status_label.configure(text="⚠️ No chapters detected.")
        return
    book_title = ab_title_var.get().strip() or "Audiobook"
    author     = ab_author_var.get().strip()
    year       = ab_year_var.get().strip()
    voice_id   = VOICES.get(ab_voice_var.get())
    if not voice_id:
        status_label.configure(text="⚠️ Pick a narrator voice.")
        return
    lang  = lang_for_voice(voice_id)
    speed = round(ab_speed.get(), 2)
    fmt   = ab_fmt_var.get()
    make_folder = fmt in ("Folder", "Both")
    make_single = fmt in ("Single MP3", "Both")
    speak_titles = bool(ab_speak_titles_var.get())
    chapters = [(c["title_var"].get().strip() or f"Chapter {i+1}", c["content"])
                for i, c in enumerate(ab_chapters)]

    folder0 = get_default_folder()
    out_dir = filedialog.askdirectory(title="Choose where to save the audiobook",
                                      initialdir=folder0 or None)
    if not out_dir:
        return
    set_default_folder(out_dir)

    cover_bytes = ab_cover["bytes"]
    cover_mime  = ab_cover["mime"]
    try:
        est = estimate_processing_time(" ".join(c[1] for c in chapters))
    except Exception:
    ...
    def progress_cb(event, **kw):
        if event == "begin":
            smooth.begin_segments(kw.get("weights") or [1])
        elif event == "chapter_start":
            i = kw.get("i", 0); ttl = kw.get("title", "")
            app.after(0, lambda: status_label.configure(
                text=f"🎧 Chapter {i+1}/{len(chapters)}: {ttl[:40]}"))
        elif event == "chapter_done":
            smooth.segment_done(kw.get("i", 0))
        elif event == "assembling":
            app.after(0, lambda: status_label.configure(text="🎧 Assembling audiobook…"))

    def run():
        global is_generating
        try:
            res = _build_audiobook(
                chapters, voice=voice_id, speed=speed, lang=lang, out_dir=out_dir,
                book_title=book_title, author=author, year=year,
                cover_bytes=cover_bytes, cover_mime=cover_mime,
...
ab_cancel_btn.configure(command=ab_cancel)
